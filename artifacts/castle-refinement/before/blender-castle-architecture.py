...
for x in [-21,21]: cturned_wall(-7,7,x,[-3.7,0,3.7])
...
cbox('palaceInterior',cmarble,0,.57,0,9.3,.14,11.6,True)
for x in [-12.725,12.725]:
    ...
    cbox('palaceInterior',cplaster,x,4.772,0,16.15,.015,11.6)
for x in [-17.5,17.5]:
    for z in [-6.275,6.275]:
        ...
        cbox('palaceInterior',cplaster,x,4.772,z,6.6,.015,.95)
cbox('palaceInterior',cmarble,0,4.91,3.9,9.4,.26,3.6,True)
cbox('palaceInterior',cmarble,0,4.91,-4.85,9.4,.26,1.7,True)
...
for i in range(4):
    front=9.5-i*.64; back=5.65; top=(i+1)*.16
    cbox('palace',ctrim,0,top/2,(front+back)/2,42-i*.22,top,front-back,True)
for sg in [-1,1]:
...
for sg in [-1,1]:
    cbox('palace',ctrim,sg*12.6,.74,7.35,14.6,.20,.46)
    cbox('palace',ctrim,sg*12.6,1.53,7.35,14.6,.17,.48)
    for i in range(29): clathe('palace',ctrim,sg*(5.55+i*.5),.84,7.35,baluster,10)
    for x in [5.35,19.9]: cbox('palace',ctrim,sg*x,1.13,7.35,.43,.84,.48,True)
