...
function box(group: Group, x: number, y: number, z: number, sx: number, sy: number, sz: number) {
  const mesh = new Mesh(cube, material);
  mesh.position.set(x, y, z); mesh.scale.set(sx, sy, sz); group.add(mesh);
}
function strip(group: Group, points: [number, number][], width: number, height: number, closed = false) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const [x, z] = points[i];
    positions.push(x - nx, .04, z - nz, x + nx, .04, z + nz,
      x + nx, height, z + nz, x - nx, height, z - nz);
  }
...
export const ground = new Group(); ground.name = 'Solid lawn';
box(ground, 0, -.3, -6, 70, .6, 90);
export const path = new Group(); path.name = 'Solid gravel';
box(path, 0, 0, 0, 1, .08, 1);
export const parterre = new Group(); parterre.name = 'Solid parterre envelopes';
...
for (const sx of [-1, 1]) for (const sz of [-1, 1]) {
  ...
  topiary.position.set(sx * 5.7, .65, sz * 5.7); topiary.scale.set(.49, .56, .49); parterre.add(topiary);
}
...
export const cypress = new Group(); cypress.name = 'Solid cypress';
const trunk = new Mesh(new CylinderGeometry(.10, .17, 1.4, 8), material); trunk.position.y = .7; cypress.add(trunk);
const crown = new Mesh(sphere, material); crown.position.y = 2.71; crown.scale.set(.64, 2.29, .64); cypress.add(crown);
export const urn = new Group(); urn.name = 'Solid stone urn';
box(urn, 0, .065, 0, .52, .13, .52);
const vessel = new Mesh(new CylinderGeometry(.44, .15, .69, 12), material); vessel.position.y = .495; urn.add(vessel);
export const gardenBorder = new Group(); gardenBorder.name = 'Solid garden wall';
box(gardenBorder, 0, .35, 0, 10, .70, .405); box(gardenBorder, 0, .77, 0, 10, .13, .53);
for (const x of [-4.88, 0, 4.88]) box(gardenBorder, x, .50, 0, .73, 1, .73);
export const bench = new Group(); bench.name = 'Solid oak bench';
box(bench, 0, .515, 0, 2.1, .065, .59); box(bench, 0, .855, -.32, 2.1, .42, .075);
for (const x of [-.76, .76]) {
  box(bench, x, .26, 0, .10, .52, .60); box(bench, x, .755, 0, .09, .1, .56);
}
